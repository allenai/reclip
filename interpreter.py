...
class Environment:
    ...
    def left_of(b1, b2):
        ...
        # error = b1.right - b2.left
        # return 1 - np.clip(error / b1.w, a_min=0, a_max=1)

    @spatial()
    def right_of(b1, b2):
        # return b1.left >= b2.right
        return (b1.right+b1.left) / 2 > (b2.right+b2.left) / 2
        # error = b2.right - b1.left
        # return 1 - np.clip(error / b1.w, a_min=0, a_max=1)
    ...
    def above(b1, b2):
        ...
        # error = b1.bottom - b2.top
        # return 1 - np.clip(error / b1.h, a_min=0, a_max=1)

    @spatial()
    def below(b1, b2):
        # return b1.top >= b2.bottom
        return (b1.bottom+b1.top) > (b2.bottom+b2.top)
        # error = b2.bottom - b1.top
        # return 1 - np.clip(error / b1.h, a_min=0, a_max=1)
